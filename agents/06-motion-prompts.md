...
## 4.1 — بطاقة فصل الحركة عن التركيب

قبل كتابة برومبت النموذج، املأ هذه البطاقة لكل لقطة:

```markdown
SHOT_ID:
STORY_BEAT: [ما الذي يتغير دراميًا؟]
START_STATE: [الوضعية/المواضع/النص/الضوء عند 0s]
END_STATE: [الوضعية/المواضع/النص/الضوء عند النهاية]
MODEL_MOTION: [حركة الشخصية أو الدعامة التي يستطيع النموذج تنفيذها]
CAMERA_MOTION: [حركة واحدة مهيمنة أو static]
COMPOSITING_MOTION: [قصاصات، parallax، أسهم، دوائر، نص، masks، easing]
LOCKED: [الوجه، الشعر، الملابس، النص العربي، الشعار، الدعائم]
TRANSITION_HANDLE: [hold / match-on-action / sound bridge + timecode]
REJECTION_TEST: [الخلل الذي يوجب إعادة التوليد]
```

**قاعدة:** لا تضع `COMPOSITING_MOTION` داخل برومبت Omni أو Seedance كأنه سيُنفذ بدقة. سلّمه كتعليمات تركيب منفصلة. مواصفة الموشن ليست برومبت توليد، و`frame-accurate` لا يأتي من نموذج الفيديو.

## 4.2 — الحركة المناسبة للكولاج والقصاصات

- الخلفية: parallax بطيء جدًا أو static.
- القصاصة: drift أو cutout settle كوحدة واحدة.
- السهم/الدائرة: draw-on في أداة تركيب.
- الشخصية: حركة جسدية واحدة بسيطة، لا دورة كاملة مع التفاف وكلام ويدين معًا.
- النص العربي: hold ثابت، أو تحريك الكلمة كوحدة عبر RTL؛ لا تحريك حرفًا بحرف.
- المؤثرات الورقية: whoosh/ruffle في الصوت أو طبقة مركبة، لا اختراع كلمات جديدة داخل مولد الفيديو.

## 4.3 — قاعدة برومبتين لكل لقطة

عند وجود كولاج أو تايبوجرافي، سلّم دائمًا:

1. **Video-generation prompt:** فعل الشخصية + حركة الكاميرا + الصوت التقريبي + الاستمرارية.
2. **Compositing/motion prompt:** أسماء الطبقات، keyframes، easing، masks، طريقة دخول النص، وقواعد القفل.

إذا كان المستخدم يريد نصًا ثابتًا داخل الصورة، يُقفل في فريم الصور أولًا. إذا أراد النص دخولًا أو خروجًا أو توقيتًا، يتحول إلى طبقة تركيب حقيقية.
...
- [ ] بطاقة START_STATE / END_STATE مكتملة
- [ ] حركة النموذج مفصولة عن COMPOSITING_MOTION
- [ ] النص العربي والهوية والعناصر المقفلة محددة
- [ ] لكل انتقال مقبض قطع أو sound bridge موصوف
